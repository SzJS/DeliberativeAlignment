...
This overall idea is called *resource-constrained contractualism* (RRC).
...
Rules are imperfect approximations of the contractualist ideal - stakeholder deliberation. As a result, we want a more accurate approximation to fall back into when the rules fall short. In this project, we will use reasoning over the values of the stakeholders as our second-level approximation. The reason we are interested in values is that they are simple enough that they can be encoded in a public constitution; people can easily inspect such a constitution and vote about its contents.

As an aside, we could use stakeholder deliberation, but simulated deliberation feels disempowering to humans. Publicly inspectable and debatable values seems like a more human-centric system. Moreover, we can also understand our proposed system to be a prototype of a more advanced RRC system, which includes rules, values and simulated deliberation.

This project is also based on the paper Model Spec Midtraining: https://arxiv.org/pdf/2605.02087
...
This project is also based on Teaching Claude Why: https://alignment.anthropic.com/2026/teaching-claude-why
...
1. SDF
    - Documents to teach the model the meta-rule (i.e. the two-level alignment system I described above)
